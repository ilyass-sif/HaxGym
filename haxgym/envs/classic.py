...
def get_reward(prev_state: EnvState, next_state: EnvState, player_idx: int, red_team: bool = True, curriculum_factor: float = 1.0):
    ...
    # 6. Goal Reward
    goal_reward = jnp.where(red_team, 
                            jnp.where(next_state.red_scored, 10.0, 0.0), 
                            jnp.where(next_state.blue_scored, 10.0, 0.0))
    # Standard concede penalty
    concede_penalty = jnp.where(red_team,
                                jnp.where(next_state.blue_scored, -10.0, 0.0),
                                jnp.where(next_state.red_scored, -10.0, 0.0))
    
    reward += goal_reward + concede_penalty
...
def env_step(rng, env_state: EnvState, params: Params, segments: SegmentParams, planes: PlaneParams, 
    ...
    new_done = (new_steps >= max_steps) | env_state.done | red_scored | blue_scored
